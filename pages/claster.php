...
class pointLocation {
    ...
    function pointInPolygon($point, $polygon, $pointOnVertex = true) {
        ...
        foreach ($polygon as $k => $vertex) {
			foreach($vertex as $vx){
            $vertices[] = $this->pointStringToCoordinates($vx);
			$vtx[]=$k;
        }
		}
...
$n=$j->properties->name;
$polygon[$n]=$j->geometry->coordinates[0];
}


$jsf=file_get_contents("files/points.geojson");
$json=json_decode($jsf);
foreach($json as $j){
$n=$j->id;
